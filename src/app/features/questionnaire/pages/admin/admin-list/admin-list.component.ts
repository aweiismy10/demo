...
export class AdminListComponent implements OnInit {
  ...
  // 2. 在建構子中注入 MatDialog 和 MatSnackBar
  constructor(
  ...
  deleteSelected() {
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      width: '350px', // 設定對話框寬度
      data: { count: selectedItems.length } // 把「選了幾筆」這個資料傳給 Dialog
    });

    // 訂閱視窗關閉後的結果 (result 會是 true 或是 undefined)
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        // 如果使用者點了「確定」(回傳 true)，才執行刪除
        selectedItems.forEach(item => {
          this.surveyService.deleteQuestionnaire(item.id).subscribe();
        });
